    ...
    for(k_ in 1:rows_){
      ...
      for(i in 1:5){
        ...
        bad_counter <- 0;isUnique_ <- F; while(isUnique_ == F){
          BreakTies <- function(x){x + stats::runif(length(x),-1e-3,1e-3)}
            ...
            if(plotRBG){
              orig_scale_im_raster <- raster::brick(
                0.0001 + (cienv$np$array(cienv$tf$cast(ds_next_in[1,t_, , ,plotBands], cienv$tf$float32) )) +
                  0*stats::runif(length(cienv$np$array(cienv$tf$cast(ds_next_in[1,t_, , ,plotBands], cienv$tf$float32 ))),
                          min = 0, max = 0.01) ) # with random jitter
              stretch <- ifelse(
                any(apply(cienv$np$array(cienv$tf$cast(ds_next_in[1,t_, , ,plotBands],cienv$tf$float32)), 3,stats::sd) < 1.),
                yes = "", no = "lin")
        ...
        for(i in 1:5){
          ...
          if(length(plotBands) >= 3){
            orig_scale_im_raster <- raster::brick( 0.0001 +
                                                     0*stats::runif(length(np$array(ds_next_in)[1, , ,plotBands]), min = 0, max = 0.01) + # random jitter
                                                     (np$array(ds_next_in)[1,,,plotBands]))
            stretch <- ifelse( any(apply(np$array(ds_next_in)[1, , ,plotBands], 3,stats::sd) < 1.), yes = "", no = "lin")
          ...
          if(grepl(typePlot,pattern = "mean")){
            ...
            #obtain image gradients
            {
              ...
              { #if(i == 1){
                nColors <- 1000
                IG_forBreaks <- IG[[1]] + stats::runif(length(IG[[1]]),-0.0000000000001, 0.0000000000001)
                gradMag_breaks <- try(sort(quantile((c(IG_forBreaks)),probs = seq(0,1,length.out = nColors),na.rm=T)),T)
                if("try-error" %in% class(gradMag_breaks)){gradMag_breaks <-  seq(-1, 1,length.out=nColors) }
                
                # pos/neg breaks should be on the same scale across observation
                IG_forBreaks <- IG[[2]] + stats::runif(length(IG[[2]]),-0.0000000000001, 0.0000000000001)
