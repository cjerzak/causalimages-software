...
AnalyzeImageConfounding <- function(
  ...
  if( XisNull ){ X <- matrix( stats::rnorm(length(obsW)*2, sd = 0.01 ), ncol = 2) }
  ...
  
  {
    ...
    if(!optimizeImageRep){
      ...
      tauHat_propensityHajek_se <- stats::sd(tauHat_propensityHajek_vec[1:nBoot], na.rm = TRUE)
    }
    ...
    if(optimizeImageRep){
      ...
      setwd(orig_wd); ImageRepresentations <- GetImageRepresentations(
        ...
        if(!justCheckIterators){
            ...
            usedKeys <- c(); for (b in seq_along(batchStarts)) {
              ...
              for(bi_ in seq_along(batchStarts_inner)){
                ...
                m <- GetTreatProb_batch_jit(ModelList, ModelList_fixed,
                                       m,
                                       x, 
                                       cienv$jax$random$split(cienv$jax$random$key(as.integer(stats::runif(1,0, 10000))),
                                                              batchSize),
                                       StateList,
                                       MPList, TRUE)[[1]]
          ...
          if(!crossFit){ 
            ...
            tauHat_propensityHajek_se <- stats::sd(tauHat_propensityHajek_vec,na.rm=T) 
          }
          if(crossFit){ 
      ...
      if(crossFit){
        ...
        tauHat_propensityHajek_se <- stats::sd(tauHat_propensityHajek_vec) /
        ...
        # generate rest of plot
          ...
          plot_index_counter <- 0; for(in_ in plot_indices){
            ...
            if(dataType == "image"){
              ...
              try_salience <- try({salience_map[salience_map>0] <- salience_map[salience_map>0] / (0.001+stats::sd(salience_map[salience_map>0]))},T)
            ...
            if(dataType == "video"){
              ...
              # GIF part 2
              salience_map[salience_map>0] <- salience_map[salience_map>0] / (0.001+stats::sd(salience_map[salience_map>0]))
              salience_map <- sign(salience_map)*log(abs(salience_map)+1)
              par(mar = margins_gif)
