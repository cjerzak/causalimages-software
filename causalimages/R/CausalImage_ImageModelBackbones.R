...
#' @usage
#'
#' GetImageRepresentations(imageKeysOfUnits, file, nWidth_ImageRep, ...)
#'
...
#' @param nWidth_ImageRep (default = `128L`) Number of embedding features output.
...
#' @param batchSize (default = `16L`) Integer specifying batch size in obtaining representations.
...
#' \itemize{
#' \item `Representations` (matrix) A matrix containingimage/video Representations, with rows corresponding to observations.
#' \item `representations_fxn` (function) The functioning performing the embedding, returned for re-use.
#' }
#'
#' @section References:
#' \itemize{
...
GetImageRepresentations <- function(
    ...
    bn_momentum = 0.9, bn_epsilon = 0.01,
    dataType = "image",
    image_dtype = "float16",
  ...
  # acquire image
  {
    setwd(orig_wd); test_ <- tf$expand_dims(GetElementFromTfRecordAtIndices( indices = 1L,
                                                             filename = file,
                                                             readVideo = useVideo,
  ...
  usedImageKeys <- c(); last_i <- 0; ok_counter <- 0; ok<-F; while(!ok){
      ...
      # get the data
      setwd(orig_wd); batch_inference <- GetElementFromTfRecordAtIndices( indices = batch_indices,
                                                            filename = file,
