...
#' @usage
#'
#' AnalyzeImageHeterogeneity(obsW, obsY, imageKeysOfUnits, file, kClust_est, ...)
#'
...
#' @param transportabilityMat (optional) A matrix with a column named `key` specifying keys to be used for generating treatment effect predictions for out-of-sample points in earth observation data settings.
...
#' @param optimizeImageRep (default = `TRUE`)
...
#' @param nWidth_ImageRep (default = `96L`) Integer specifying the image/image sequence embedding dimension.
#' @param nDepth_Dense (default = `1L`) Hidden depth of dense layers. Default of `0L` means a single projection layer is performed after the convolutional layer (i.e., no hidden layers are used).
...
AnalyzeImageHeterogeneity <- function(obsW,
...

  {
    ...
    # (ar <- tfp$layers$DenseReparameterization(1L))(jnp$array(as.matrix(1),tf$float16))
    ...
    variable_dtype <- otherData_dtypes <- jnp$float32
  ...
  for(BAYES_STEP in c(1)){
    ...
    if(BAYES_STEP == 1){
      ...
      d_prior <- oryx$distributions$Normal(loc = jnp$zeros(shape, keras_layers_dtype), scale = cnst(SD_PRIOR_MODEL))
    ...
    if(BAYES_STEP == 2){
      ...
      PRIOR_MODEL_FXN <- function(name_){
        ...
        if( ZERO_LEN_IN){
          prior_loc_name <- "jnp$zeros(shape, dtype = keras_layers_dtype)"; prior_SD_name <- "1"
        }
        if( !ZERO_LEN_IN){
  ...
  if(!c("monti") %in% ls(envir = globalenv())){
  ...
  if( plotResults == T){
    ...
    for(typePlot in (typePlot_vec <- c("mean","mean_upperConf","uncertainty"))){
      ...
      plotting_coordinates_mat_ <- ifelse(dataType == "video",
                             yes = list(video_plotting_fxn),
                             no = list(image_plotting_fxn))[[1]]()
