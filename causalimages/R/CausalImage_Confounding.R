...
AnalyzeImageConfounding <- function(
                                   ...
                                   seed = NULL,
                                   quiet = F){
  print2("Establishing computational environment...")
  {
...

  {
    ...
    NORM_SD <- NORM_MEAN <- c(); for(momentCalIter in 1:(momentCalIters<-10)){
      ...
      if(is.null(NORM_MEAN)){
        NORM_MEAN <- NORM_SD <- apply(as.array(ds_next_train[[1]]),ApplyAxis,sd); NORM_MEAN[] <- NORM_SD[] <- 0
      }
      ...
      setwd(orig_wd); ImageRepresentations <- GetImageRepresentations(
        ...
        # define optimizer and training step
        {
          ...
          print2(sprintf("Total trainable parameter count: %s",
                         nTrainable <- sum(unlist(lapply(jax$tree_leaves(ModelParams), function(zer){zer$size})))))
        ...
        n_sgd_iters <- nSGD; tauMeans <- c();i_<-1L ; DoneUpdates <- 0L; for(i in i_:nSGD){
          ...
          v_and_grad_loss_jax <- try(GradAndLossAndAux(
            ...
            MPList, # MPlist
            F), T) # inference
          if("try-error" %in% class(v_and_grad_loss_jax)){browser()}
        ...
        for(cut_ in unique(KeyQuantCuts)){ # use when not incorporating X's
          ...
          if( any(zer %% 10 == 0) | 1 %in% zer ){ print2(sprintf("Proportion done: %.3f", atP)) }
          {
            setwd(orig_wd); ds_next_in <- GetElementFromTfRecordAtIndices(
                                                indices = which(unique(imageKeysOfUnits) %in% unique(imageKeysOfUnits)[zer]),
                                                filename = file,
      ...
      makePlots <- function(){
            ...
            width = length(plot_indices)*5+2,height = nrows_im*5)
        {
          ...
          plot_index_counter <- 0; for(in_ in plot_indices){
            ...
            # get data
            setwd(orig_wd); ds_next_in <- GetElementFromTfRecordAtIndices(
                                                    indices = which(unique(imageKeysOfUnits) %in% imageKeysOfUnits[in_]),
                                                    filename = file,
                                                    readVideo = useVideo,
    ...
    if( optimizeImageRep ){
        ...
        for(keyNum_ in sample(1:length(unique(imageKeysOfUnits)), 25, replace = T)){
          ...
          {
            setwd(orig_wd); ds_next_in <- GetElementFromTfRecordAtIndices( indices = keyNum_,
                                                           filename = file,
                                                           readVideo = useVideo,
